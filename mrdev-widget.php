...
if(is_admin()) {
	...
	if (strpos($url,'taxonomy=category') !== false || strpos($url,'taxonomy=post_tag') !== false) {
		function mrdev_add_style() {
			...
			wp_enqueue_style( 'mrdevWidget_admin', plugin_dir_url( __DIR__ ).'mrdev-widget/assets/css/admin.css',array(),'0.9.431');
		}
		...
		if(isset($instance['widgetid'])) {
			...
			*/
			if(!wp_script_is('mrdev_utils','registered')) {
				wp_register_script( 'mrdev_utils', plugin_dir_url( __DIR__ ).'mrdev-widget/tools/mr-utils/js/utils.js',array(),'0.9.431');
			}
			if(file_exists($cache_dir.'/mrdev/widgets/js/'.$widgetid.'.js') && !wp_script_is('mrdev_widget')) {
				wp_enqueue_script( 'mrdev_widget', $cache_url.'/mrdev/widgets/js/'.$widgetid.'.js', array('mrdev_utils'),'0.9.431');
			} else {
				wp_deregister_script('mrdev_widget');
				wp_enqueue_script( 'mrdev_widget', plugin_dir_url( __DIR__ ).'mrdev-widget/assets/js/widget.js', array('mrdev_utils'),'0.9.431');
			}
			/*
			...
			*/
			if(!wp_style_is('mrdev_utils','registered')) {
				wp_register_style( 'mrdev_utils', plugin_dir_url( __DIR__ ).'mrdev-widget/tools/mr-utils/css/utils.css',array(),'0.9.431');
			}
			...
			if(!wp_style_is('mrdev_utils_desktop','registered')) {
				...
				wp_register_style( 'mrdev_utils_desktop', plugin_dir_url( __DIR__ ).'mrdev-widget/tools/mr-utils/css/utils-desktop.css',array(),'0.9.431',${"mrdev_breakpoint_desktop"});
			}
			if(!wp_style_is('mrdev_utils_tablet','registered')) {
				...
				wp_register_style( 'mrdev_utils_tablet', plugin_dir_url( __DIR__ ).'mrdev-widget/tools/mr-utils/css/utils-tablet.css',array(),'0.9.431',${"mrdev_breakpoint_tablet"});
			}
			if(!wp_style_is('mrdev_utils_phone','registered')) {
				...
				wp_register_style( 'mrdev_utils_phone', plugin_dir_url( __DIR__ ).'mrdev-widget/tools/mr-utils/css/utils-phone.css',array(),'0.9.431',${"mrdev_breakpoint_phone"});
			}*/
			if(file_exists($cache_dir.'/mrdev/widgets/css/'.$widgetid.'.css')) {
				wp_enqueue_style( $widgetid.'_css', $cache_url.'/mrdev/widgets/css/'.$widgetid.'.css', array('mrdev_utils'),'0.9.431');
			} else {
				wp_enqueue_style( 'mrdev_widget', plugin_dir_url( __DIR__ ).'mrdev-widget/assets/css/widget.css', array('mrdev_utils'),'0.9.431');
				if($theme == "default") {
					//Official Themes
					wp_enqueue_style( 'mrdev_'.$theme.'_css', plugin_dir_url( __DIR__ ).'mrdev-widget/themes/'.$theme.'/'.$theme.'.css',array('mrdev_widget'),'0.9.431');
				} else if($theme == "none") {
				} else {
					//Custom Themes
					wp_enqueue_style( 'mrdev_'.$theme.'_css', get_home_url().'/wp-content/themes/mrdev/widget/themes/'.$theme.'/'.$theme.'.css',array('mrdev_widget'),'0.9.431');
				}
				/*
				Custom CSS
				*/
				if(file_exists($cache_dir.'/mrdev/widgets/css/'.$widgetid.'_custom.css')) {
					wp_enqueue_style( $widgetid.'_customcss', $cache_url.'/mrdev/widgets/css/'.$widgetid.'_custom.css','0.9.431');
				}
			}
	...
	public function form( $instance ) {
		...
		<?php
			...
			<?php
				...
				wp_enqueue_media();
				wp_enqueue_script( 'mrdevWidget_admin', plugin_dir_url( __DIR__ ).'mrdev-widget/assets/js/admin.js', array('jquery'),'0.9.431');
			?>
			</div>
			<?php
	}
