...
document.addEventListener("click", function (event) {
  if (event.target.matches(".mr-admin details:not([open]) .mr-section")) {
    event.target
      .closest(".mr-admin")
      .querySelector("input.lastactivedetails").value =
      event.target.parentElement.getAttribute("class");
    var mrwidDetails = document.querySelectorAll(
      ".mr-admin details:not([active])"
    );
    for (var id = 0; id < mrwidDetails.length; id++) {
      var mrwidDetail = mrwidDetails[id];
      if (
        !mrwidDetail.classList.contains("mr-parentscontainer") &&
        mrwidDetail !== event.target
      ) {
    ...
    event.target.addEventListener("change", function (event) {
      mrSlideUp(
        event.target.closest(".mr-admin").querySelector(".mr-themeoptions")
      );
      mrSlideDown(
        event.target.closest(".mr-admin").querySelector(".mr-savetheme")
      );
    });
  } else if (
    event.target.matches(".mr-parents input") ||
    event.target.matches(".mr-notchilds input")
  ) {
    event.target.addEventListener("change", function (event) {
      mrSlideDown(
        event.target
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 0) {
        event.target
          .closest(".mr-admin")
          .querySelector(".mr-list")
          .classList.add("including");
        mrSlideUp(
          event.target
            .closest(".mr-admin")
            .querySelector(".mr-groupexcludecontainer")
        );
        mrSlideUp(
          event.target
            .closest(".mr-admin")
            .querySelector(".mr-parentscontainer")
        );
      } else {
        if (
          event.target
            .closest(".mr-admin")
            .querySelector(".mr-list")
            .classList.contains("including")
        ) {
          event.target
            .closest(".mr-admin")
            .querySelector(".mr-list")
            .classList.remove("including");
        }
        mrSlideDown(
          event.target
            .closest(".mr-admin")
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 9) {
        mrSlideDown(
          event.target.closest(".mr-admin").querySelector(".mr-fallbackimage")
        );
        var imageoverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-imageoverride");
        for (var id = 0; id < imageoverrides.length; id++) {
          mrSlideDown(imageoverrides[id]);
        }
        var titleoverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-titleoverride");
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.matches(":checked")) {
        mrSlideDown(
          event.target.closest(".mr-admin").querySelector(".mr-imagessize")
        );
      } else {
        mrSlideUp(
          event.target.closest(".mr-admin").querySelector(".mr-imagessize")
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 1) {
        ...
        var textoverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-textoverride");
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 1) {
        var linkoverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-linkoverride");
        for (var id = 0; id < linkoverrides.length; id++) {
          mrSlideDown(linkoverrides[id]);
        }
        mrSlideDown(
          event.target.closest(".mr-admin").querySelector(".mr-bottomlinktext")
        );
      } else {
        var linkoverrides = event.target
          .closest(".mr-admin")
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 0) {
        var dateoverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-dateoverride");
    ...
    event.target.addEventListener("change", function (event) {
      if (event.target.value != 0) {
        var authoroverrides = event.target
          .closest(".mr-admin")
          .querySelectorAll(".mr-authoroverride");
