...
function mrwidChangePage(currentElement,mrwidLayout,mrwidPage) {
	if(!!mrwidLayout.querySelector('.mr-pageselect option[value="'+mrwidPage+'"]')) {
		...
		setTimeout(function() {
			...
			if(mrwidLayout.querySelector('.mr-page'+(parseInt(mrwidPage)+1)) && mrwidLayout.querySelector('.mr-below')) {
...
function mrwidBelow(currentElement) {
	...
	let mrwidNewPage = mrwidLayout.querySelector('.mr-page'+(parseInt(mrwidPage)+1));
	mrwidLayout.classList.remove('mr-transitionright','mr-transitionleft');
	if(!mrwidNewPage) {
		...
		let mrwidCurrentRadio = mrwidLayout.querySelector('.mr-radio[value="'+(parseInt(mrwidPage)+1)+'"]');
		if(mrwidCurrentRadio) {
			mrwidCurrentRadio.setAttribute('checked','checked');
		}
		if(!mrwidLayout.querySelector('.mr-page'+(parseInt(mrwidPage)+2)) && mrwidLayout.querySelector('.mr-below')) {
			mrwidLayout.querySelector('.mr-below').classList.add('mr-hide');
		}
	}
	setTimeout(function() {
...
document.addEventListener('click',function(event) {
	if (event.target.matches('.mr-layout:not(.mr-hover) .mr-pages a[href]:not([href="#"]):not([href="javascript:void(0)"]):not([target="_blank"])')) {
		const mrwidNotThis = event.target.closest('.mr-layout').querySelectorAll('.mr-item');
		const mrwidThisTabs = event.target.closest('.mr-layout').querySelectorAll('.mr-tab');
		const mrwidThisID = event.target.closest('.mr-item').className.split(" ")[0];
		if(mrwidNotThis) {
		...
		event.stopPropagation();
	} /*end*/ /*pagetoggles-4*/ else if (event.target.matches('button.mr-scroll')) {
		const currentElement = event.target;
		if(!currentElement.classList.contains('loading')) {
			currentElement.classList.add('loading');
			mrwidBelow(currentElement);
